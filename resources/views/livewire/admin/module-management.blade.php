                            ...
                            <div class="grid grid-cols-3 gap-2 mb-2">
                                @foreach ($this->moduleAssignments as $assignment)
                                    ...
                                    <div
                                        ...
                                        class="relative flex flex-col items-center gap-1.5 p-3 rounded-lg text-center transition-colors border
                                            {{ $isUpcoming    ? 'opacity-40 border-transparent bg-gray-50 dark:bg-gray-700/50' : '' }}
                                            {{ $isSelected    ? 'border-blue-400 bg-blue-50 dark:bg-blue-900/20' : '' }}
                                            {{ ! $isUpcoming && ! $isSelected ? 'border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600' : '' }}
                                        "
                                    >
                                        {{-- Three-dot menu button -- only for non-upcoming --}}
                                        @if (! $isUpcoming)
                                            <div class="absolute top-1.5 right-1.5" @click.stop>
                                                <button
                                                    type="button"
                                                    @click="menuOpen = !menuOpen"
                                                    class="w-5 h-5 flex items-center justify-center rounded text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
                                                ...
                                                <div
                                                    ...
                                                    x-transition:enter-end="opacity-100 scale-100"
                                                    class="absolute right-0 top-6 z-50 w-36 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg py-1"
                                                >
                                                    <button
                                                        type="button"
                                                        wire:click="showAssignmentDetail({{ $assignment->id }})"
                                                        @click="menuOpen = false"
                                                        class="w-full text-left px-3 py-1.5 text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                                                    >
                                                        Details
                                                    </button>
                                                    <button
                                                        ...
                                                        @click="menuOpen = false"
                                                        class="w-full text-left px-3 py-1.5 text-xs text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                                                    >
                                                        Delete
                                                    </button>
                                                </div>
                                        ...
                                        {{-- Card body -- clickable area only for non-upcoming --}}
                                        <div
                                            @if (! $isUpcoming) wire:click="selectAssignment({{ $assignment->id }})" @endif
                                            class="{{ ! $isUpcoming ? 'cursor-pointer w-full' : 'w-full' }}"
                                        >
                                            ...
                                            <p class="text-xs font-medium text-gray-800 dark:text-gray-200 truncate w-full leading-tight">
                                                {{ $assignment->title }}
                                            </p>

                                            <p class="text-xs {{ $isPast ? 'text-red-400' : 'text-gray-400' }} mt-0.5">
                                                Due {{ $assignment->due_at->format('d M') }}
                                            </p>

                                            <span class="inline-flex items-center mt-1 px-1.5 py-0.5 rounded-sm text-xs font-medium
                                ...
                                <table class="w-full text-xs">
                                    ...
                                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700/50">
                                        @forelse ($this->moduleSubmissions as $row)
                                            ...
                                            <tr
                                                @if ($hasSubmission)
                                                    wire:click="showGradePopup({{ $row->id }})"
                                                    class="cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/20 transition-colors {{ $hasSubmission ? '' : 'opacity-50' }}"
                                                @else
                                                    class="opacity-50"
                                                @endif
                                            >
                                                ...
                                                <td class="px-3 py-2 {{ $hasSubmission ? 'text-gray-700 dark:text-gray-300' : 'text-gray-300 dark:text-gray-600' }} truncate max-w-28">
                                                    @if ($hasSubmission && $row->file_name)
                                                        <a
                                                            href="{{ route('submissions.download', $row->id) }}"
                                                            class="text-blue-500 hover:underline truncate block max-w-28"
                                                            title="{{ $row->file_name }}"
                                                            @click.stop
                                                        >
                                                            {{ $row->file_name }}
                ...
                <div class="p-5 flex flex-col gap-3">
                    <div class="grid grid-cols-2 gap-3">
                        @foreach ([
                            ...
                            ['Max score',      $a->max_score ?? 'Unscored'],
                            ['Created by',     $a->creator?->name ?? '—'],
                            ['Available from', $a->available_at->format('d M Y, H:i')],
                            ['Due at',         $a->due_at->format('d M Y, H:i')],
                            ['Created at',     $a->created_at->format('d M Y, H:i')],
                            ['Updated at',     $a->updated_at->format('d M Y, H:i')],
                        ] as [$label, $value])
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 rounded-lg px-3 py-2">
                        ...
                        <div class="text-xs text-gray-800 dark:text-gray-200 whitespace-pre-wrap">{{ $a->description }}</div>
                    </div>
                </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 dark:bg-black/70">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md mx-4 border border-gray-200 dark:border-gray-700">
                ...
                <div class="p-5 flex flex-col gap-4">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            @foreach ([
                                ...
                                ['File',         $sub->file_name],
                                ['Submitted at', $sub->submitted_at?->format('d/m/Y H:i:s') ?? '—'],
                                ['Processed at', $sub->processed_at?->format('d/m/Y H:i:s') ?? '—'],
                            ] as [$label, $value])
                                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg px-3 py-2">
                                    <div class="text-xs text-gray-400 mb-0.5">{{ $label }}</div>
                    ...
                    {{-- Grade info --}}
                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wide mb-2">Grade</p>
                        @if ($grade)
                            <div class="grid grid-cols-2 gap-2">
                                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg px-3 py-2 col-span-2">
                                    <div class="text-xs text-gray-400 mb-0.5">Score</div>
                                    <div class="text-sm font-semibold text-gray-900 dark:text-gray-100">
                                        {{ $grade->score !== null ? $grade->score : 'No score' }}
                                        @if ($sub->assignment?->max_score)
                                            <span class="text-xs font-normal text-gray-400">/ {{ $sub->assignment->max_score }}</span>
                                        @endif
                                    </div>
                                </div>
                                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg px-3 py-2">
                                    <div class="text-xs text-gray-400 mb-0.5">Graded by</div>
                                    <div class="text-xs font-medium text-gray-900 dark:text-gray-100">{{ $grade->grader?->name ?? '—' }}</div>
                                </div>
                                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg px-3 py-2">
                                    <div class="text-xs text-gray-400 mb-0.5">Graded at</div>
                                    <div class="text-xs font-medium text-gray-900 dark:text-gray-100">{{ $grade->graded_at?->format('d/m/Y H:i') ?? '—' }}</div>
                                </div>
                                @if ($grade->feedback)
                                    <div class="bg-gray-50 dark:bg-gray-700 rounded-lg px-3 py-2 col-span-2">
                                        <div class="text-xs text-gray-400 mb-1">Feedback</div>
                                        <div class="text-xs text-gray-800 dark:text-gray-200 whitespace-pre-wrap">{{ $grade->feedback }}</div>
                                    </div>
                                @endif
                            </div>
                        @else
                            <div class="flex items-center justify-center py-6 text-gray-400 dark:text-gray-500 text-xs">
                                Not yet graded
                            </div>
                        @endif
                    </div>
                ...
                <div class="flex justify-end gap-2 px-5 py-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    @endif
                    <button wire:click="closeGradePopup" class="px-3 py-1.5 text-xs border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-400 transition-colors">
                        Close
                    </button>
                </div>
            </div>
